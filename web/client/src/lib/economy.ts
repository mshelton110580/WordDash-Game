...
const SETTINGS_STORAGE_KEY = 'worddash_settings';
...
const STATS_STORAGE_KEY = 'worddash_stats';
...
export interface GameStats {
  ...
  levelBestTimes: Record<number, number>; // levelNumber -> best time remaining (higher = faster)
...
export interface LevelStatsUpdate {
  levelNumber: number;
  wordsFound: string[];
  score: number;
  stars: number;
...
export function updateStatsOnLevelComplete(update: LevelStatsUpdate): void {
  const stats = loadStats();
  stats.totalWordsFound += update.wordsFound.length;
  ...
  // Level-specific
  const prev = stats.levelBestScores[update.levelNumber] || 0;
  if (update.score > prev) stats.levelBestScores[update.levelNumber] = update.score;

  const prevStars = stats.levelStars[update.levelNumber] || 0;
  if (update.stars > prevStars) {
    stats.levelStars[update.levelNumber] = update.stars;
    if (update.stars > 0) stats.levelsCompleted = Math.max(stats.levelsCompleted, Object.keys(stats.levelStars).filter(k => (stats.levelStars[parseInt(k)] || 0) > 0).length);
  }

  if (update.goalType === 'scoreTimed') {
    const prevTime = stats.levelBestTimes[update.levelNumber] || 0;
    if (update.timeRemaining > prevTime) stats.levelBestTimes[update.levelNumber] = update.timeRemaining;
  }

  if (update.maxStreak > stats.bestStreak) stats.bestStreak = update.maxStreak;
  if (update.maxCascade > stats.bestCascade) stats.bestCascade = update.maxCascade;

  for (const word of update.wordsFound) {
